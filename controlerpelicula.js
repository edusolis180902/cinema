var Item = require('./pelicula'); //se inica llamando la clase constructor y llamanddo a la funcion del cosntructor
module.exports = class _Pelicula {
   constructor( ) { // porque el profe quiso dejarlo asi xddxddxdxdxdxdx  (vacio)

   }
Guardar(req,res) { //se inica la funcion guardar con un requesy y respond para que haga el pedido a la base de datos 
	Item.create(
			{
  NOMBRE: req.body.NOMBRE,     //dentro del item create se llama un request que tiene  los obejtos y dentro la lista gigantes de todos los asientos que son objetos 
    ...
    ESTADO: req.body.ESTADO
			...
			function(err, item) { //se llama una funcion para que si hay  un error lo muestre y que si no responda y  tire todas las peliculas
				if (err)
                    {
					res.send(err);}
